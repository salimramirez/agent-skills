...
Writes the four layers of one context, wired for a single CRUD aggregate:
    ...
    <into>/<context>/
      domain/model/<entity>.entity.js
      application/<context>.store.js
      infrastructure/<entity>.assembler.js, <context>-api.js
      presentation/<context>-routes.js,
                   views/<entity>-list.vue, views/<entity>-form.vue
...
Examples (run from the root of the Vue app, SKILL being this skill's directory):
...
def main():
    parser = argparse.ArgumentParser(
        description="Scaffold a DDD bounded context for a Vue app.",
    ...
    if resolved == SKILL_DIR or SKILL_DIR in resolved.parents:
        print(
            ...
            f"Run this from the root of your Vue app, for example:\n"
...
Three things the generator cannot do for you:
  ...
  1. Add the endpoint path to every .env file -- .env.development and
     .env.production both declare the same keys:
         VITE_{mapping['__ENTITIES_UPPER__']}_ENDPOINT_PATH="/{mapping['__entities-kebab__']}"

  2. Mount the context in router.js:
         import {context_camel}Routes from './{context_kebab}/presentation/{context_kebab}-routes.js';
         {{ path: '/{context_kebab}', name: '{context_kebab}', children: {context_camel}Routes }},
