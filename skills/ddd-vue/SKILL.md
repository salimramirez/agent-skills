...
- **Loosens:** repositories are API endpoints rather than aggregate stores, so the repository interface the core puts in the domain layer has no counterpart here; aggregates and value objects are lighter or skipped (the UI rarely needs them); "domain events" are usually reactive state updates rather than published events.
...
- **The domain layer imports nothing from Vue.** No `ref`, no router, no axios.
- **Views are smart and routed; components are dumb and reusable** — `defineProps` in, `defineEmits` out, and a dumb component never sees the store.
- **Every route is named**, lazy-loaded, and owned by its context; navigation goes by name, never by a hand-built path.
- **URLs are composed from `import.meta.env`**, one base URL per provider plus one path per endpoint, declared identically in every `.env` file.
- **Classes, exported functions and store state carry JSDoc.** It is the type system.
...
- **Adding a resource end to end** — the seven files in order, plus the checklist to finish on. Start here for "add X to the app". Read [adding-a-resource.md](references/adding-a-resource.md)
...
- **House style** — JSDoc as the type system, member ordering, and the layer smells to avoid. Read [house-style.md](references/house-style.md)
...
- **The CRUD path** — assemblers, endpoints, and the context gateway. Read [infrastructure.md](references/infrastructure.md)
...
- **The Pinia store** — refs and computeds, errors and loaded flags, stitching, and the light `reactive()` variant. Read [state-store.md](references/state-store.md)

**Building the UI**

- **Views, components, and routing** — the smart/dumb split, named lazy routes, and the app shell. Read [presentation.md](references/presentation.md)
- **Forms and writes** — one view for create and edit, validation as UX, entity or command. Read [forms.md](references/forms.md)
- **Cross-cutting concerns** — guards and interceptors in the context that owns the rule, localization, and bootstrap order. Read [cross-cutting.md](references/cross-cutting.md)

## Copy instead of retyping

Two things ship as code, because they should come out the same every time.

**Run both commands from the root of the Vue app**, with `$SKILL` pointing at the directory this skill was installed into — `.claude/skills/ddd-vue` in a standard install:

```bash
SKILL=.claude/skills/ddd-vue
```

- **`assets/shared-kernel/`** — `base-api.js` and `base-endpoint.js`, laid out in the layer they belong to. Copy the folder's contents into `src/shared/`:
...
- **`scripts/new-context.py`** — scaffolds a whole bounded context wired for one CRUD aggregate, deriving every spelling of the name from two arguments:
  ...
  `--into` is resolved against the current directory, so run this from the app root, not from the skill folder. Add `--dry-run` to see what it would write, `--plural People` when the naive plural is wrong. It prints the two edits it cannot make for you: the `.env` keys and the route in `router.js`. What it writes is ordinary code — read it, then model the real aggregate.

For Vue questions the references do not cover, read the documentation at `https://vuejs.org`.
