...
- **`WebSecurityConfiguration.filterChain`** — the `permitAll` list is the public surface; CORS allows every origin, which is right for a class project and wrong for production. Both are three lines in one method. Keep the line that permits the `ERROR` dispatch: without it an unhandled exception on any endpoint comes back as a 401 instead of a 500, which sends you looking for a token problem that does not exist.
- **Authorization beyond authentication** — `@EnableMethodSecurity` is already on, so `@PreAuthorize("hasRole('ADMIN')")` on a controller method is all it takes to restrict it. The asset restricts nothing by role on purpose; that is a domain decision.
