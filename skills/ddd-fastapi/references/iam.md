...
1. `POST /api/v1/authentication/sign-up` with `{"username", "password", "roles": ["ROLE_ADMIN"]}` → the service resolves the role names (`Role.from_name`, an unknown one is a `DomainError` → **400**), hashes the password, builds the `User` — which trims the username — and only then checks that the **trimmed** username is free (`UsernameTakenError` → **409**), then saves → **201** with `{id, username, roles}`. No roles means the default one, `ROLE_USER`. Checking the raw input instead was a real bug: with `ana` taken, `" ana"` passed the check and hit the unique column as a 500.
2. `POST /api/v1/authentication/sign-in` with `{"username", "password"}` → the service loads the user and verifies the hash; an unknown username and a wrong password raise the **same** `InvalidCredentialsError` → **401**, so the response never says which one it was. Nor does its timing: for an unknown username the service still computes a hash, so both paths cost one Argon2 run — measured, a median of 32 ms against 34 ms; without it, the unknown username answered about 30 ms faster. Success is **200** with `{id, username, token}`.
