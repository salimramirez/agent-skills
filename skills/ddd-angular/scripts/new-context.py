...
Writes the four layers of one context, wired for a single CRUD aggregate:
    ...
    <into>/<context>/
      ...
      infrastructure/<entities>-response.ts, <entity>-assembler.ts,
                     <entities>-api-endpoint.ts, <context>-api.ts
      presentation/<context>.routes.ts, views/<entity>-list/, views/<entity>-form/
...
def main():
    parser = argparse.ArgumentParser(
        description="Scaffold a DDD bounded context for an Angular app.",
    ...
    parser.add_argument("--into", default="src/app",
                        help="directory holding the contexts (default: src/app)")
...
Three things the generator cannot do for you:
  ...
  1. Add the endpoint path to every environments/environment*.ts:
         platformProvider{mapping['__Entities__']}EndpointPath: '/{mapping['__entities-kebab__']}',

  2. Mount the context in app.routes.ts:
         const {context_camel}Routes = () =>
           import('./{context_kebab}/presentation/{context_kebab}.routes')
             .then(m => m.{context_camel}Routes);
         {{path: '{context_kebab}', loadChildren: {context_camel}Routes}},
  ...
  3. Model the aggregate. The generated entity has one `name` field as a
     ...
     experts use, and update the resource and the assembler to match.
""".rstrip())
    return 0
